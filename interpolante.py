...
def g(x,y,z,x0,y0,z0):                                  #trilinear interpolating function
    ...
    while (i<2):
        ...
        while (j<2):
            k=0       
            while (k<2):                                
                r=r+(-1)**(i+j+k-1)*(x-x0[l+abs(i-1)])*(y-y0[m+abs(j-1)])*(z-z0[n+abs(k-1)])*f(x0[l+i],y0[m+j],z0[n+k]) 
                k=k+1                                   #f values should be tabulated in a list: F[l+i,m+j,n+k]
            j=j+1
        i=i+1
    r=r/((x0[l+1]-x0[l])*(y0[m+1]-y0[m])*(z0[n+1]-z0[n]))
    return r
    
a=1.05                                                  #point to estimate the function in
b=1.35
c=0.72

d1=100                                                  #no. of divisions of dominium axis
d2=200
d3=150

o1=0                                                    #lower extremum of dominium axis
o2=0
o3=0

s1=4                                                    #upper extremum of dominium axis
s2=6
s3=3
